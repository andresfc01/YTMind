  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Sección de documentos (solo si hay un agentId) */}
        {agent && agent.id && (
          <div className="mt-6 space-y-6">
            <div className="border-t border-gray-200 pt-5">
              <h3 className="text-lg font-medium leading-6 text-gray-900">Documentos</h3>
              <p className="mt-1 text-sm text-gray-500">
                Gestionar los documentos que el agente utilizará como contexto.
              </p>
            </div>
            <DocumentManager agentId={agent.id} />
          </div>
        )}

        {/* Sección de URLs (solo si hay un agentId) */}
        {agent && agent.id && (
          <div className="mt-6 space-y-6">
            <div className="border-t border-gray-200 pt-5">
              <h3 className="text-lg font-medium leading-6 text-gray-900">URLs</h3>
              <p className="mt-1 text-sm text-gray-500">Gestionar las URLs que el agente utilizará como contexto.</p>
            </div>
            <UrlManager agentId={agent.id} />
          </div>
        )}
      </form>
    </div>
