 ...
 */
export async function listChannelVideos({ channelIdentifier, includeShorts = false }) {
  try {
    // Primero obtener la información básica del canal (sin pedir videos populares para evitar recursión)
    const channelInfoResult = await getChannelInfo({ channelIdentifier, fetchPopularVideos: false });
    const channelId = channelInfoResult.channelId;

    // Construir la URL para la API de búsqueda
    // Solicitamos más resultados para compensar los que filtraremos
    const maxResults = includeShorts ? 10 : 50;
    const apiUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelId}&type=video&order=viewCount&maxResults=${maxResults}&key=${process.env.YOUTUBE_API_KEY}`;
    ...
    // Guardar solo los IDs de videos en el canal
    if (!channelInfoResult.analysis) {
      channelInfoResult.analysis = {};
    }
    channelInfoResult.analysis.popularVideos = finalResults.map((video) => video.videoId);

    // Verificar si el canal existe en la base de datos y guardarlo/actualizarlo
    try {
      const existingChannel = await ChannelRepository.findByChannelId(channelId);
      if (existingChannel) {
        console.log(`Actualizando canal "${channelInfoResult.name}" (${channelId}) con videos populares...`);
      } else {
        console.log(`Guardando canal "${channelInfoResult.name}" (${channelId}) con videos populares...`);
      }
      await saveChannelToDatabase(channelInfoResult);
    } catch (dbError) {
      console.error("Error al guardar canal en la base de datos:", dbError);
      // Continuamos para devolver los videos aunque haya error en la BD
    }

    // Asegurarnos de no devolver las transcripciones en la respuesta
    const responseVideos = finalResults.map((video) => {
      // Crea una copia del objeto sin la propiedad transcription
      const { transcription, ...videoWithoutTranscription } = video;
